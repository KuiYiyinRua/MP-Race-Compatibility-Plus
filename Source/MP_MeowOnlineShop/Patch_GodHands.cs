...
namespace MP_MeowOnlineShop
{
    ...
    internal static class Patch_GodHands
    {
        ...
        public static Type PoseDebuggerType;
        ...
        public static void Apply(Harmony harmony)
        {
            ...
            MethodInfo preDraw = AccessTools.Method(typeof(PawnTweener), "PreDrawPosCalculation");
            MethodInfo tweenerPrefix = AccessTools.Method(typeof(Patch_GodHands), nameof(PawnTweenerPreDrawPrefix));
            if (preDraw != null && tweenerPrefix != null)
            {
                try
                {
                    harmony.Patch(preDraw, prefix: new HarmonyMethod(tweenerPrefix));
                }
                catch (Exception e)
                {
                    Log.Warning($"[MP-MeowOnlineShop] God Hands pawn preview patch failed: {e.Message}");
                }
            }

            if (PoseDebuggerType != null)
            {
                ConstructorInfo poseCtor = AccessTools.Constructor(PoseDebuggerType);
                MethodInfo posePostfix = AccessTools.Method(typeof(Patch_GodHands), nameof(PoseDebuggerCtorPostfix));
                if (poseCtor != null && posePostfix != null)
                {
                    try
                    {
                        harmony.Patch(poseCtor, postfix: new HarmonyMethod(posePostfix));
                    }
                    catch (Exception e)
                    {
                        Log.Warning($"[MP-MeowOnlineShop] God Hands pose debugger window patch failed: {e.Message}");
                    }
                }
            }
        ...
        private static bool PawnTweenerPreDrawPrefix(PawnTweener __instance)
        {
            return !GodHandSync.IsLocalPreviewPawn(__instance);
        }

        private static void PoseDebuggerCtorPostfix(Window __instance)
        {
        ...
        public static void Resolve()
        {
            ...
            PoseDebuggerType = AccessTools.TypeByName("GodHandMod.Window_PoseDebugger");
    ...
    internal static class GodHandSync
    {
        ...
        internal struct PrecisionTransformArgs
        {
            public float Ox;
            public float Oy;
        ...
        private static readonly HashSet<int> LocalPreviewPawnIds = new HashSet<int>();
        private static AccessTools.FieldRef<object, Pawn> TweenerPawnRef;
        ...
        public static void Register()
        {
            ...
            RegisterMethod(nameof(SyncGodHandForceReleaseAt));
        ...
        private static void RegisterSyncWorkers()
        {
            try
            {
                MP.RegisterSyncWorker<Patch_GodHandsDesignators.CleaningOptions>(
                    CleanOptionsSyncer,
                    typeof(Patch_GodHandsDesignators.CleaningOptions),
                    false,
                    false);
            }
            catch (Exception e)
            {
                Log.Warning(
                    "[MP-MeowOnlineShop] God Hands cleaning-options SyncWorker " +
                    "registration failed: " + e);
            }

            try
            {
                MP.RegisterSyncWorker<PrecisionTransformArgs>(
                    PrecisionTransformSyncer,
                    typeof(PrecisionTransformArgs),
                    false,
                    false);
            }
            catch (Exception e)
            {
                Log.Warning(
                    "[MP-MeowOnlineShop] God Hands precision-transform SyncWorker " +
                    "registration failed: " + e);
            }
        }

        private static void CleanOptionsSyncer(
            SyncWorker sync,
            ref Patch_GodHandsDesignators.CleaningOptions options)
        {
            if (sync.isWriting)
            {
                bool enableHealing = options.EnableHealing;
                sync.Bind(ref enableHealing);
                float healAmount = options.HealAmount;
                sync.Bind(ref healAmount);
                bool enableTending = options.EnableTending;
                sync.Bind(ref enableTending);
                bool enableRegrowth = options.EnableRegrowth;
                sync.Bind(ref enableRegrowth);
                bool removeScars = options.RemoveScars;
                sync.Bind(ref removeScars);
                bool cureAddiction = options.CureAddiction;
                sync.Bind(ref cureAddiction);
                bool enablePlantGrowth = options.EnablePlantGrowth;
                sync.Bind(ref enablePlantGrowth);
                float plantGrowthBonus = options.PlantGrowthBonus;
                sync.Bind(ref plantGrowthBonus);
                bool enableFoodPreservation = options.EnableFoodPreservation;
                sync.Bind(ref enableFoodPreservation);
                bool enableRepair = options.EnableRepair;
                sync.Bind(ref enableRepair);
                return;
            }

            bool readEnableHealing = false;
            sync.Bind(ref readEnableHealing);
            float readHealAmount = 0f;
            sync.Bind(ref readHealAmount);
            bool readEnableTending = false;
            sync.Bind(ref readEnableTending);
            bool readEnableRegrowth = false;
            sync.Bind(ref readEnableRegrowth);
            bool readRemoveScars = false;
            sync.Bind(ref readRemoveScars);
            bool readCureAddiction = false;
            sync.Bind(ref readCureAddiction);
            bool readEnablePlantGrowth = false;
            sync.Bind(ref readEnablePlantGrowth);
            float readPlantGrowthBonus = 0f;
            sync.Bind(ref readPlantGrowthBonus);
            bool readEnableFoodPreservation = false;
            sync.Bind(ref readEnableFoodPreservation);
            bool readEnableRepair = false;
            sync.Bind(ref readEnableRepair);
            options = new Patch_GodHandsDesignators.CleaningOptions
            {
                EnableHealing = readEnableHealing,
                HealAmount = readHealAmount,
                EnableTending = readEnableTending,
                EnableRegrowth = readEnableRegrowth,
                RemoveScars = readRemoveScars,
                CureAddiction = readCureAddiction,
                EnablePlantGrowth = readEnablePlantGrowth,
                PlantGrowthBonus = readPlantGrowthBonus,
                EnableFoodPreservation = readEnableFoodPreservation,
                EnableRepair = readEnableRepair
            };
        }

        private static void PrecisionTransformSyncer(
            SyncWorker sync,
            ref PrecisionTransformArgs args)
        {
            if (sync.isWriting)
            {
                float ox = args.Ox;
                ...
                float oy = args.Oy;
                sync.Bind(ref oy);
                float rotation = args.Rotation;
                sync.Bind(ref rotation);
                float sx = args.Sx;
                sync.Bind(ref sx);
                float sy = args.Sy;
                sync.Bind(ref sy);
                ...
                sync.Bind(ref layer);
                int overrideRot = args.OverrideRot;
                sync.Bind(ref overrideRot);
                return;
            }

            float readOx = 0f;
            sync.Bind(ref readOx);
            float readOy = 0f;
            sync.Bind(ref readOy);
            float readRotation = 0f;
            sync.Bind(ref readRotation);
            float readSx = 1f;
            sync.Bind(ref readSx);
            float readSy = 1f;
            sync.Bind(ref readSy);
            float readSz = 1f;
            sync.Bind(ref readSz);
            int readLayer = 0;
            sync.Bind(ref readLayer);
            int readOverrideRot = -1;
            sync.Bind(ref readOverrideRot);
            args = new PrecisionTransformArgs
            {
                Ox = readOx,
                Oy = readOy,
        ...
        private static void RegisterMethod(string name)
        {
            ...
            {
                Log.Warning(
                    $"[MP-MeowOnlineShop] God Hands sync registration failed for {name}: {e}");
            }
        }
        ...
        public static void PreviewGodHandDragLocal(int playerId, int mapIndex, int cellX, int cellZ)
        {
            if (playerId < 0 || !GodHandSessions.TryGetValue(new SessionKey(mapIndex, playerId), out GodHandDragSession session))
                return;
            Map map = Patch_GodHands.FindMap(mapIndex);
            ...
            // preview cell keeps force-cancel drops in place on every peer.
            Vector3 mousePos = cell.ToVector3Shifted();
            foreach (KeyValuePair<int, Pawn> pair in session.Pawns)
            {
                Pawn p = pair.Value;
                if (p == null || p.Destroyed || p.Dead)
                    continue;
                if (!session.Offsets.TryGetValue(pair.Key, out Vector3 offset))
                    offset = Vector3.zero;
                UpdatePawnTweener(p, mousePos + offset);
                LocalPreviewPawnIds.Add(pair.Key);
            }
        }

        public static bool IsLocalPreviewPawn(PawnTweener tweener)
        {
            if (tweener == null || LocalPreviewPawnIds.Count == 0)
                return false;
            if (TweenerPawnRef == null)
                TweenerPawnRef = AccessTools.FieldRefAccess<Pawn>(typeof(PawnTweener), "pawn");
            return TweenerPawnRef != null &&
                   TweenerPawnRef(tweener) is Pawn p &&
                   LocalPreviewPawnIds.Contains(p.thingIDNumber);
        }

        public static bool TryGetLocalGodHandDragCell(int playerId, int mapIndex, out IntVec3 cell)
        {
        ...
        private static void ClearLocalPreview()
        {
            LocalPreviewPawnIds.Clear();
        }
        ...
        public static void SyncGodHandRelease(int playerId, int mapIndex, int cellX, int cellZ)
        {
            ...
            foreach (var pair in session.Pawns)
            {
                ...
                if (p.Spawned)
                {
                    ...
                    UpdatePawnTweener(p, targetCell.ToVector3Shifted());
                }
                p.jobs?.EndCurrentJob(JobCondition.InterruptForced);
            }
        ...
        {
            SyncGodHandForceReleaseAt(playerId, mapIndex, IntVec3.Invalid.x, IntVec3.Invalid.z);
        }

        public static void SyncGodHandForceReleaseAt(int playerId, int mapIndex, int cellX, int cellZ)
        {
            SessionKey key = new SessionKey(mapIndex, playerId);
        ...
        private static void AbortGodHandDrag(GodHandDragSession session, Map map, IntVec3 abortCell)
        {
            foreach (Pawn p in session.Pawns.Values)
            {
                if (p == null || p.Destroyed || p.Dead)
                    continue;
                if (abortCell.IsValid && abortCell.InBounds(map))
                {
                    if (!session.Offsets.TryGetValue(p.thingIDNumber, out Vector3 offset))
                        offset = Vector3.zero;
                    IntVec3 target = (abortCell.ToVector3Shifted() + offset).ToIntVec3();
                    if (target.InBounds(map) && p.Spawned)
                    {
                        p.Position = target;
                        p.Notify_Teleported(false, true);
                        UpdatePawnTweener(p, target.ToVector3Shifted());
                    }
                }
                p.jobs?.EndCurrentJob(JobCondition.InterruptForced);
            }
            ...
            foreach (Thing t in session.Things.Values)
            {
                ...
                if (abortCell.IsValid && abortCell.InBounds(map))
                {
                    Vector3 offset = session.Offsets.TryGetValue(t.thingIDNumber, out Vector3 stored)
                        ? stored
                        : Vector3.zero;
                    IntVec3 candidate = (abortCell.ToVector3Shifted() + offset).ToIntVec3();
        ...
        public static void SyncCleanArea(
            int mapIndex,
            int cellX,
            int cellZ,
            int mode,
            int radius,
            Patch_GodHandsDesignators.CleaningOptions options)
        {
            Map map = Patch_GodHands.FindMap(mapIndex);
            ...
            object oldEnableHealing = Patch_GodHands.GetSettingsValue<object>("cleaningEnableHealing", false);
            object oldHealAmount = Patch_GodHands.GetSettingsValue<object>("cleaningHealAmount", 0f);
            object oldEnableTending = Patch_GodHands.GetSettingsValue<object>("cleaningEnableTending", false);
            object oldEnableRegrowth = Patch_GodHands.GetSettingsValue<object>("cleaningEnableRegrowth", false);
            object oldRemoveScars = Patch_GodHands.GetSettingsValue<object>("cleaningRemoveScars", false);
            object oldCureAddiction = Patch_GodHands.GetSettingsValue<object>("cleaningCureAddiction", false);
            object oldEnablePlantGrowth = Patch_GodHands.GetSettingsValue<object>("cleaningEnablePlantGrowth", false);
            object oldPlantGrowthBonus = Patch_GodHands.GetSettingsValue<object>("cleaningPlantGrowthBonus", 0f);
            object oldEnableFoodPreservation =
                Patch_GodHands.GetSettingsValue<object>("cleaningEnableFoodPreservation", false);
            object oldEnableRepair = Patch_GodHands.GetSettingsValue<object>("cleaningEnableRepair", false);
            ...
            Patch_GodHands.SetSettingsValue("cleaningEnableHealing", options.EnableHealing);
            Patch_GodHands.SetSettingsValue("cleaningHealAmount", options.HealAmount);
            Patch_GodHands.SetSettingsValue("cleaningEnableTending", options.EnableTending);
            Patch_GodHands.SetSettingsValue("cleaningEnableRegrowth", options.EnableRegrowth);
            Patch_GodHands.SetSettingsValue("cleaningRemoveScars", options.RemoveScars);
            Patch_GodHands.SetSettingsValue("cleaningCureAddiction", options.CureAddiction);
            Patch_GodHands.SetSettingsValue("cleaningEnablePlantGrowth", options.EnablePlantGrowth);
            Patch_GodHands.SetSettingsValue("cleaningPlantGrowthBonus", options.PlantGrowthBonus);
            Patch_GodHands.SetSettingsValue("cleaningEnableFoodPreservation", options.EnableFoodPreservation);
            Patch_GodHands.SetSettingsValue("cleaningEnableRepair", options.EnableRepair);

            int state = 0;
            Map mapForPop = null;
            int seed = Gen.HashCombineInt(
                // Map.Index is a local collection index and can differ between
                // peers after map load/removal. The sync command already carries
                // the stable uniqueID, so use it for the cleaning Rand scope too.
                map.uniqueID,
                Gen.HashCombineInt(
                    cellX,
                    Gen.HashCombineInt(
                        ...
                        Gen.HashCombineInt(
                            mode,
                            Gen.HashCombineInt(radius, Find.TickManager.TicksGame)))));
            DeterministicRandScope.Begin(map, seed, 0x4C1E, ref state, out mapForPop, ignoreGate: true);
            try
            {
            ...
            finally
            {
                ...
                Patch_GodHands.SetSettingsValue("cleaningEnableHealing", oldEnableHealing);
                Patch_GodHands.SetSettingsValue("cleaningHealAmount", oldHealAmount);
                Patch_GodHands.SetSettingsValue("cleaningEnableTending", oldEnableTending);
                Patch_GodHands.SetSettingsValue("cleaningEnableRegrowth", oldEnableRegrowth);
                Patch_GodHands.SetSettingsValue("cleaningRemoveScars", oldRemoveScars);
                Patch_GodHands.SetSettingsValue("cleaningCureAddiction", oldCureAddiction);
                Patch_GodHands.SetSettingsValue("cleaningEnablePlantGrowth", oldEnablePlantGrowth);
                Patch_GodHands.SetSettingsValue("cleaningPlantGrowthBonus", oldPlantGrowthBonus);
                Patch_GodHands.SetSettingsValue("cleaningEnableFoodPreservation", oldEnableFoodPreservation);
                Patch_GodHands.SetSettingsValue("cleaningEnableRepair", oldEnableRepair);
            }
        }
        ...
        public static void SyncPrecisionTransform(
            int mapIndex,
            int thingId,
            string componentKey,
            PrecisionTransformArgs args)
        {
            Map map = Patch_GodHands.FindMap(mapIndex);
            Thing thing = Patch_GodHands.FindThingById(map, thingId);
            if (map == null || thing == null)
                return;
            Vector2 offset = new Vector2(args.Ox, args.Oy);
            Vector3 scale = new Vector3(args.Sx, args.Sy, args.Sz);
            int? rot = args.OverrideRot < 0 ? (int?)null : args.OverrideRot;
            if (string.IsNullOrEmpty(componentKey))
            {
                Patch_GodHands.InvokeStatic(
                    Patch_GodHands.TransformManagerSetMethod,
                    thing,
                    offset,
                    args.Rotation,
                    scale,
            ...
            else if (Patch_GodHands.TransformDataType != null)
            {
                ...
                if (data != null)
                {
                    ...
                    Patch_GodHands.SetField(data, AccessTools.Field(Patch_GodHands.TransformDataType, "rotation"), args.Rotation);
                    Patch_GodHands.SetField(data, AccessTools.Field(Patch_GodHands.TransformDataType, "scale"), scale);
                    Patch_GodHands.SetField(data, AccessTools.Field(Patch_GodHands.TransformDataType, "drawLayer"), args.Layer);
