...
namespace MP_MeowOnlineShop
{
    ...
    internal static class Patch_GodHandsDesignators
    {
        ...
        private static readonly Dictionary<int, IntVec3> LastCleanCell = new Dictionary<int, IntVec3>();
        private static readonly Dictionary<int, int> LastCleanTick = new Dictionary<int, int>();

        internal struct CleaningOptions
        {
            public bool EnableHealing;
            public float HealAmount;
            public bool EnableTending;
            public bool EnableRegrowth;
            public bool RemoveScars;
            public bool CureAddiction;
            public bool EnablePlantGrowth;
            public float PlantGrowthBonus;
            public bool EnableFoodPreservation;
            public bool EnableRepair;
        }
        ...
        public static void Apply(Harmony harmony)
        {
            ...
            TryPatch(harmony, Patch_GodHands.CleaningControllerType, "CleanupProcessedThings",
                nameof(CleanupProcessedThingsPrefix), Priority.First + 2);
        ...
        private static bool CleaningDesignateSingleCellPrefix(object __instance, IntVec3 c)
        {
            ...
            CleaningOptions options = CaptureCleaningOptions();
            GodHandSync.SyncCleanArea(
        ...
        private static bool CleanAreaPrefix(Map map, IntVec3 center)
        {
            ...
            if (LastCleanCell.TryGetValue(map.uniqueID, out IntVec3 last) &&
                last == center &&
                LastCleanTick.TryGetValue(map.uniqueID, out int lastTick) &&
                tick - lastTick < 2)
                return false;
            LastCleanCell[map.uniqueID] = center;
            LastCleanTick[map.uniqueID] = tick;
            CleaningOptions options = CaptureCleaningOptions();
            GodHandSync.SyncCleanArea(
                map.uniqueID,
                center.x,
        ...
        private static bool CleanupProcessedThingsPrefix()
        {
        ...
        private static CleaningOptions CaptureCleaningOptions()
        {
            return new CleaningOptions
            {
                EnableHealing = Patch_GodHands.GetSettingsValue("cleaningEnableHealing", false),
                HealAmount = Patch_GodHands.GetSettingsValue("cleaningHealAmount", 0f),
                EnableTending = Patch_GodHands.GetSettingsValue("cleaningEnableTending", false),
                EnableRegrowth = Patch_GodHands.GetSettingsValue("cleaningEnableRegrowth", false),
                RemoveScars = Patch_GodHands.GetSettingsValue("cleaningRemoveScars", false),
                CureAddiction = Patch_GodHands.GetSettingsValue("cleaningCureAddiction", false),
                EnablePlantGrowth = Patch_GodHands.GetSettingsValue("cleaningEnablePlantGrowth", false),
                PlantGrowthBonus = Patch_GodHands.GetSettingsValue("cleaningPlantGrowthBonus", 0f),
                EnableFoodPreservation = Patch_GodHands.GetSettingsValue("cleaningEnableFoodPreservation", false),
                EnableRepair = Patch_GodHands.GetSettingsValue("cleaningEnableRepair", false)
            };
        }
    }
}
