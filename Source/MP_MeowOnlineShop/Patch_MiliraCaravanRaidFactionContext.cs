...
{
    /// <summary>
    /// Desync-243/244: The Tale of Milira's ruin-outpost raid outcome queues a
    ...
    /// Milira arrival action bypasses that path. Singleplayer is untouched.
    /// </summary>
    internal static class Patch_MiliraCaravanRaidFactionContext
    {
        ...
        private static readonly string[] OutpostTypeNames =
        {
        ...
        internal static void Apply(Harmony harmony)
        {
            ...
            int patched = 0;
            for (int i = 0; i < OutpostTypeNames.Length; i++)
            {
                try
                {
                    Type outpostType = AccessTools.TypeByName(OutpostTypeNames[i]);
                    ...
                    if (raidLambda == null || caravanField == null)
                    {
                        Log.Warning(
                            ...
                            OutpostTypeNames[i] + "; raid can still desync.");
                        continue;
                    }
                ...
                {
                    Log.Warning(
                        LogTag + " apply failed for " + OutpostTypeNames[i] +
                        ": " + e.Message);
                }
            }
            ...
            Log.Message(
                ...
                $"patched={patched}/{OutpostTypeNames.Length}, " +
                "caravan faction and stable home-map context applied to pawn/raid generation.");
        }
