...
namespace MP_MeowOnlineShop
{
    ...
    internal static class Patch_GodHandsControllers
    {
        ...
        private static bool GodHandDragPrefix(object __instance, Map map, IntVec3 cell, float time)
        {
            ...
            if (playerId < 0)
                return true;
            // Network-friendly drag: the movement process stays local, only the
            // release cell is synchronized. Preview follows the mouse on the
            // issuing client without touching authoritative pawn positions.
            GodHandSync.PreviewGodHandDragLocal(playerId, map.uniqueID, cell.x, cell.z);
            return false;
        }
        ...
        private static bool WrenchSnatchPrefix(object __instance, Map map, Thing target, Pawn user)
        {
            if (Patch_GodHands.PausedGodHandsBlocked())
                return false;
            if (!ShouldInterceptUi() || map == null || target == null)
                return true;
            GodHandSync.SyncWrenchSnatchTurret(map.uniqueID, target.thingIDNumber);
            return false;
        }

        private static bool WrenchRemoveFromPawnPrefix(object __instance, Map map, Pawn pawn, object wornHead)
        {
            if (Patch_GodHands.PausedGodHandsBlocked())
                return false;
            if (!ShouldInterceptUi() || map == null || pawn == null || wornHead == null)
                return true;
            int headId = wornHead is Thing thing ? thing.thingIDNumber : 0;
            if (headId == 0)
                return true;
            GodHandSync.SyncWrenchRemoveFromPawn(map.uniqueID, pawn.thingIDNumber, headId);
            return false;
        }
        ...
        private static bool PrecisionApplyChangesPrefix(object __instance)
        {
            ...
            GodHandSync.SyncPrecisionTransform(
                ...
                thing.thingIDNumber,
                key,
                new GodHandSync.PrecisionTransformArgs
                {
                    Ox = offset.x,
                    Oy = offset.y,
                    Rotation = rotation,
